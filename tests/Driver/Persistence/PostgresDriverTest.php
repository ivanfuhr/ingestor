...
SQL);
    ...
    #[Test]
    public function it_synchronizes_staging_identity_sequences_after_prefilled_stage_copy(): void
    {
        $this->pdo->exec('DROP TABLE IF EXISTS associados_contas CASCADE');
        $this->pdo->exec(<<<'SQL'
CREATE TABLE associados_contas (
...
SQL);
        ...
        for ($id = 1; $id <= 685; ++$id) {
            $this->pdo->exec(sprintf(
                "INSERT INTO associados_contas (id, conta_id, associado_id) OVERRIDING SYSTEM VALUE VALUES (%d, 'conta-%d', 'assoc-%d')",
                $id,
                $id,
                $id,
            ));
    ...
    #[Test]
    public function it_synchronizes_staging_generated_always_identity_sequences_after_prefilled_stage_copy(): void
    {
        $this->pdo->exec('DROP TABLE IF EXISTS associados_contas CASCADE');
        $this->pdo->exec(<<<'SQL'
CREATE TABLE associados_contas (
